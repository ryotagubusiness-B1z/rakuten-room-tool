...
for (const name of [
  ...
  "CAROTE カローテ 卵焼き フライパン 選べる2点セット",
  "CAROTE カローテ 卵焼き フライパン 2点セット選べる",
  "CAROTE カローテ 卵焼き フライパン 2点セットよりどり",
  "CAROTE カローテ 卵焼き フライパン 2点セットから選べる",
  "CAROTE カローテ 卵焼き フライパン 選べる 2点 セット",
  "CAROTE カローテ 卵焼き フライパン 2 点セットから選べる"
]) {
...
const aliasPan = {name:"CAROTE カローテ 玉子焼き器 IH対応", genre:"キッチン・日用品"};
assert.equal(summarizeProduct(aliasPan).subject, "卵焼きフライパン", "玉子焼き器の別名を正規カテゴリへ寄せる");
assert.equal(/CAROTE|カローテ/i.test(genIntro(aliasPan,"polite")), false, "別名商品でもブランド名を本文へ漏らさない");

const alcoholFree = {name:"アルコール フリー 化粧水 敏感肌用", genre:"美容・コスメ"};
const alcoholSummary = summarizeProduct(alcoholFree);
assert.equal(alcoholSummary.subject, "化粧水");
assert.equal(alcoholSummary.features.includes("アルコールフリー"), true, "分離されたフリー表記を意味を保って結合する");
assert.equal(alcoholSummary.features.includes("アルコール"), false, "フリー表記を落として逆の意味にしない");

const cicaCream = {name:"CICA セラミド クリーム", genre:"美容・コスメ"};
assert.equal(summarizeProduct(cicaCream).features.includes("セラミド"), true, "英字語の後ろにある正当なカタカナ仕様をブランド扱いしない");

const glutenFreeBread = {name:"グルテン フリー パン 国産米粉", genre:"食品・健康"};
const breadSummary = summarizeProduct(glutenFreeBread);
assert.equal(breadSummary.subject, "パン", "食品のフリー表記でも商品種別を逆転させない");
assert.equal(breadSummary.features.includes("グルテンフリー"), true, "食品でも否定語を落とさず意味を保持する");
assert.equal(/注目したいグルテン/.test(genIntro(glutenFreeBread,"oshi")), false, "グルテンを商品種別として紹介しない");

const sugarFreeChocolate = {name:"シュガー フリー チョコレート 国産", genre:"食品・健康"};
assert.equal(summarizeProduct(sugarFreeChocolate).subject, "チョコレート", "未登録ジャンルでもフリー対象語を商品種別にしない");
assert.equal(summarizeProduct(sugarFreeChocolate).features.includes("シュガーフリー"), true, "未登録ジャンルでもフリー表記を保持する");

const genericBrand = {name:"ABC エービーシー 化粧水 高保湿", genre:"美容・コスメ"};
assert.equal(summarizeProduct(genericBrand).features.includes("エービーシー"), false, "一般ブランドの英字＋読み仮名を仕様にしない");
assert.equal(/ABC|エービーシー/.test(genIntro(genericBrand,"polite")), false, "一般ブランド名を本文へ漏らさない");

const unknownCategoryBrand = {name:"ABC エービーシー チョコレート", genre:"食品・健康"};
assert.equal(summarizeProduct(unknownCategoryBrand).subject, "チョコレート", "未登録カテゴリでも英字ブランドの読み仮名を商品種別にしない");

const cicaHerb = {name:"CICA ツボクサ クリーム", genre:"美容・コスメ"};
assert.equal(summarizeProduct(cicaHerb).features.includes("ツボクサ"), true, "英字語の後ろにある未登録の正当なカタカナ仕様を落とさない");

const brunoToaster = {name:"BRUNO ブルーノ トースター", genre:"家電・ガジェット"};
assert.equal(summarizeProduct(brunoToaster).features.includes("ブルーノ"), false, "一般ブランドの英字＋カタカナ読みを仕様にしない");
assert.equal(/ブルーノが特徴/.test(genIntro(brunoToaster,"polite")), false, "ブランド読みを不自然な特徴文にしない");

for (const name of ["パンプス ローヒール", "パンケーキ 米粉", "シャンパン 辛口"]) {
  assert.notEqual(summarizeProduct({name,genre:""}).subject, "パン", `${name}: 部分一致だけでパンにしない`);
}

const splitChoiceSet = {name:"CAROTE カローテ 卵焼き フライパン 2点セット から選べる",genre:"キッチン・日用品"};
assert.deepEqual(Array.from(summarizeProduct(splitChoiceSet).features), [], "分離した『から選べる』もセット情報として除外する");
